...
unsigned char
get_elf_symbol_char_type_x64(struct s_elf_symbol_header elf_symbol_header, struct s_elf_info elf_info)
{
/*
	R	Read-only global symbol.
	r	Read-only local symbol.
...
*/
	...
	type = ELF64_ST_TYPE(elf_symbol_header.symbol_type);
	bind = ELF64_ST_BIND(elf_symbol_header.symbol_type);
	...
	section_header = (Elf64_Shdr *)(elf_info.start_of_file + elf_info.section_header_offset);
...
int
get_elf_symbol_header_x64(struct s_elf_symbol_header *elf_symbol_header, size_t index,
	struct s_elf_section_header elf_section_header, struct s_elf_info elf_info)
{
	...
	Elf64_Sym *symbol_header;

	symbol_header = (Elf64_Sym *)(elf_info.start_of_file + elf_section_header.offset + (index * elf_section_header.entry_size));
...
int
get_elf_section_header_x64(struct s_elf_section_header *elf_section_header, size_t index, struct s_elf_info elf_info)
{
...
int
get_elf_program_header_x64(struct s_elf_program_header *elf_program_header, size_t index, struct s_elf_info elf_info)
{
	...
	program_header = (Elf64_Phdr *)(elf_info.start_of_file + elf_info.program_header_offset);
...
void
get_elf_info_x64(struct s_elf_info *elf_info)
{
	Elf64_Ehdr	*header;

	header = (Elf64_Ehdr *)elf_info->start_of_file;
	elf_info->elf_class = ELFCLASS64;
	...
	section_header = (Elf64_Shdr *)(elf_info->start_of_file + header->e_shoff);
