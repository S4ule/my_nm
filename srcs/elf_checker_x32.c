...
static int
elf_program_header_check_x32(Elf32_Ehdr *header, struct s_file_info file_info)
{
	...
	// Program Header
	if (header->e_phentsize != sizeof(Elf32_Phdr) && header->e_phnum != 0) {
	...
	program_header = (Elf32_Phdr *)(file_info.data + header->e_phoff);

	section_header = (Elf32_Shdr *)(file_info.data + header->e_shoff);
	if (header->e_phnum >= PN_XNUM) {
...
static int
elf_section_header_check_x32(Elf32_Ehdr *header, struct s_file_info file_info)
{
	Elf32_Shdr	*section_header;
	size_t		e_shnum;

	// Section Header
	if (header->e_shentsize != sizeof(Elf32_Shdr) && header->e_shnum != 0) {
...
static int
elf_string_header_check_x32(Elf32_Ehdr *header, struct s_file_info file_info)
{
	Elf32_Shdr		*section_header;
	Elf32_Shdr		*string_section_header;
	size_t			e_shnum;
	size_t			name_len;
...
static int
elf_symbol_table_check_x32(Elf32_Ehdr *header, struct s_file_info file_info)
{
	Elf32_Shdr		*section_header;
	Elf32_Shdr		*string_section_header;
	Elf32_Sym		*symbole_table;
	...
	for (size_t i = 0; i < e_shnum; i++)
	{
			...
			section_header[i].sh_type == SHT_DYNSYM )
		{
			...
			symbole_table = (Elf32_Sym *)(section_header[i].sh_offset + file_info.data);
			...
			for (size_t j = 0; j < symbole_table_size; j++)
			{
				...
				unsigned char st_type = ELF32_ST_TYPE(symbole_table[j].st_info);
				unsigned char st_bind = ELF32_ST_BIND(symbole_table[j].st_info);
				...
				unsigned char st_other = ELF32_ST_VISIBILITY(symbole_table[j].st_other);
...
int
elf_check_x32(struct s_file_info file_info)
{
	Elf32_Ehdr	*header;
	int			err;

	header = (Elf32_Ehdr *)file_info.data;
	...
	err = elf_program_header_check_x32(header, file_info);
	if (err != 0) {
		return err;
	}
	err = elf_section_header_check_x32(header, file_info);
	if (err != 0) {
		return err;
	}

	err = elf_string_header_check_x32(header, file_info);
	if (err != 0) {
		return err;
	}
	err = elf_symbol_table_check_x32(header, file_info);
	if (err != 0) {
		return err;
	}
